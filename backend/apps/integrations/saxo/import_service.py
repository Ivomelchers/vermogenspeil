...
def _asset_type_for_symbol(symbol: str, transaction_type: str) -> str:
    """Infer asset type from symbol and transaction type."""
    ...
    # Saxo uses numeric UICs for stocks/ETFs
    if symbol.isdigit():
...
def _category_for_asset_type(asset_type: str) -> str:
    """Map asset type to tax category."""
...
@transaction.atomic
def import_saxo_csv_for_user(
    user,
    file_content: str,
    *,
    label: str = "Saxo Bank (CSV)",
    parse_result: CsvParseResult | None = None,
    source_filename: str = "",
    column_mapping: dict | None = None,
    ai_used: bool = False,
) -> dict:
    """Import Saxo Bank CSV export."""
    result = parse_result or parse_saxo_csv(
        file_content,
    ...
    for row in rows:
        ...
        _, created = Transaction.objects.get_or_create(
            ...
            defaults={
                ...
                "source_platform": PlatformType.SAXO,
